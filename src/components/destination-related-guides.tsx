...
export function RelatedGuides({ slugs, destinationName }: RelatedGuidesProps) {
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-8">
      <h2 className="font-display mb-4 text-2xl font-extrabold text-ink">Guias relacionados</h2>
      ...
      <ul className="space-y-3">
        {posts.map((post) => post && (
          <li key={post.slug}>
            <Link
              href={`/blog/${post.slug}`}
              className="block rounded-lg border border-ink/8 bg-surface-raised p-4 transition hover:border-primary hover:shadow-sm"
            >
